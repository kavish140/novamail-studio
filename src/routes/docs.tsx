...
export const Route = createFileRoute("/docs")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Quickstart, authentication, and reference for the NovaMail transactional email API." },
      { property: "og:title", content: "NovaMail API Documentation" },
      { property: "og:description", content: "Everything you need to integrate NovaMail in minutes." },
    ],
  }),
  component: DocsPage,
});
...
function DocsPage() {
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-6 py-14 lg:grid-cols-[220px_1fr]">
        <aside className="hidden lg:block">
          <div className="sticky top-24">
            <div className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">On this page</div>
            <nav className="space-y-1 text-sm">
              {sections.map((s) => (
                <a key={s.id} href={`#${s.id}`} className="block rounded-md px-3 py-1.5 text-muted-foreground transition hover:bg-surface hover:text-foreground">
                  {s.label}
                </a>
              ))}
            </nav>
        ...
        <article className="prose-invert max-w-3xl space-y-16">
          <header id="intro">
            ...
            <h1 className="font-display text-4xl font-semibold tracking-tight">NovaMail API documentation</h1>
            <p className="mt-4 text-muted-foreground">
              The NovaMail API is a REST API that lets you send transactional email from any language that can make an HTTPS request.
              All endpoints are versioned and live under <code className="rounded bg-surface px-1.5 py-0.5 text-sm">https://api.novamail.app/v1</code>.
            </p>
          </header>
          ...
          <Section id="auth" title="Authentication">
            <p className="text-muted-foreground">
              ...
              <Link to="/dashboard/keys" className="text-primary underline-offset-4 hover:underline">API Keys</Link> page and pass it in the <code>Authorization</code> header.
            </p>
            <CodeBlock filename="terminal" language="bash" code={`curl https://api.novamail.app/v1/email \\
  -H "Authorization: Bearer nm_live_••••••••"`} />
            <p className="text-sm text-muted-foreground">
              Never expose live keys in client code. Use a <code>nm_test_</code> key for local development and CI.
            </p>
          </Section>
          ...
          <Section id="send" title="Send email — POST /v1/email">
            <p className="text-muted-foreground">Sends a single transactional email and returns a message id.</p>
            ...
            <CodeBlock filename="200 OK" language="json" code={`{
  "id": "msg_2lQ7v8x4hKpC",
  "to": "ada@lovelace.dev",
  "status": "queued",
  "created_at": "2026-06-06T09:42:11Z"
          ...
          <Section id="webhooks" title="Webhooks">
            <p className="text-muted-foreground">Subscribe to lifecycle events — <code>email.delivered</code>, <code>email.opened</code>, <code>email.clicked</code>, <code>email.bounced</code> — and we'll POST signed payloads to your endpoint.</p>
            <CodeBlock filename="webhook payload" language="json" code={`{
  ...
  "data": {
    ...
    "delivered_at": "2026-06-06T09:42:14Z"
          ...
          <Section id="errors" title="Errors">
            <p className="text-muted-foreground">NovaMail uses standard HTTP status codes and returns a structured error envelope.</p>
            <CodeBlock filename="422 Unprocessable" language="json" code={`{
  "error": {
